...
import {
  Framework,
  FrameworkType,
} from "../compliance/entities/framework.entity";
import { ComplianceProgress } from "../compliance/entities/compliance-progress.entity";
import {
  Task,
  TaskStatus,
  TaskPriority,
} from "../compliance/entities/task.entity";
import {
  Document,
  DocumentStatus,
...
export class SeederService {
  ...
  async seed(): Promise<void> {
    ...
    try {
      ...
      const progressRecords = await this.createComplianceProgress(
        organization,
        frameworks
      );
  ...
  private async createFrameworks(): Promise<Framework[]> {
    const frameworksData = [
      ...
      },
      {
        ...
        requirements:
          "ISO 27001:2022 Annex A controls for information security management systems",
        categories: [
  ...
  private async createTasks(
    ...
    const tasksData = [
      ...
      // ISO 27001 Tasks
      {
        ...
        requirements: ["A.5.1", "A.5.2"],
      },
      {
        name: "Risk Assessment and Treatment",
        description:
          "Conduct information security risk assessment and implement treatment plans",
        priority: TaskPriority.CRITICAL,
        status: TaskStatus.PENDING,
        dueDate: new Date("2024-08-25"),
        frameworkName: FrameworkType.ISO27001,
        requirements: ["A.5.7", "A.5.8"],
      },
      {
        name: "Access Control Management",
        description:
          "Implement access control policies and user access management",
        priority: TaskPriority.HIGH,
        status: TaskStatus.PENDING,
        dueDate: new Date("2024-09-05"),
        frameworkName: FrameworkType.ISO27001,
        requirements: ["A.8.1", "A.8.2", "A.8.3"],
      },
      {
        ...
        frameworkName: FrameworkType.ISO27001,
        requirements: ["A.8.24", "A.8.25"],
      },
      {
        name: "Physical Security Controls",
        description:
          "Establish physical security perimeters and access controls",
        priority: TaskPriority.MEDIUM,
        status: TaskStatus.IN_PROGRESS,
        dueDate: new Date("2024-08-28"),
        frameworkName: FrameworkType.ISO27001,
        requirements: ["A.7.1", "A.7.2", "A.7.3"],
      },
      {
        ...
        frameworkName: FrameworkType.ISO27001,
        requirements: ["A.5.24", "A.5.25", "A.5.26"],
      },
    ];
    ...
    for (const taskData of tasksData) {
      try {
        const framework = frameworks.find(
          (f) => f.name === taskData.frameworkName
        );
        if (!framework || !framework.id) {
        ...
        if (!progress) {
          console.error(
            `Progress record not found for framework: ${framework.name}`
          );
          continue;
        }
    ...
    for (const progress of progressRecords) {
      ...
      progress.completedTasks = tasks.filter(
        (t) => t.status === TaskStatus.COMPLETED
      ).length;
      progress.inProgressTasks = tasks.filter(
        (t) => t.status === TaskStatus.IN_PROGRESS
      ).length;
      progress.pendingTasks = tasks.filter(
  ...
  private async createDocuments(
    ...
    for (const docData of documentsData) {
      const framework = frameworks.find(
        (f) => f.name === docData.frameworkName
      );
