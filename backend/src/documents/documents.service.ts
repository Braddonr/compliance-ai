...
export class DocumentsService {
  ...
  async findOne(id: string): Promise<Document> {
    const document = await this.documentsRepository.findOne({
      where: { id },
      relations: ['framework', 'createdBy', 'collaborators', 'organization', 'versions'],
    });
  ...
  async update(id: string, updateDocumentDto: UpdateDocumentDto, userId: string): Promise<Document> {
    ...
    if (updateDocumentDto.content && updateDocumentDto.content !== oldContent) {
      ...
      const newVersionNumber = this.incrementVersion(lastVersion?.version || '1.0.0');
  ...
  async removeCollaborator(documentId: string, userId: string): Promise<Document> {
    const document = await this.findOne(documentId);
    document.collaborators = document.collaborators.filter(c => c.id !== userId);
    await this.documentsRepository.save(document);
    return this.findOne(documentId);
  }
  ...
  async getByFramework(frameworkId: string, organizationId?: string): Promise<Document[]> {
    const queryBuilder = this.documentsRepository
      .createQueryBuilder('document')
      .leftJoinAndSelect('document.framework', 'framework')
      .leftJoinAndSelect('document.createdBy', 'createdBy')
